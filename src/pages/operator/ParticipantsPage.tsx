...
import { HiOutlineMagnifyingGlass, HiOutlineFunnel, HiOutlineArrowDownTray, HiOutlineChevronLeft, HiOutlineChevronRight } from 'react-icons/hi2';
...
const mockParticipants = Array.from({ length: 25 }, (_, i) => ({
  ...
  status: ['registered', 'winner', 'redeemed'][Math.floor(Math.random() * 3)] as 'registered' | 'winner' | 'redeemed',
...
export default function ParticipantsPage() {
  ...
  const filtered = mockParticipants.filter(
    (p) => p.name.toLowerCase().includes(search.toLowerCase()) || p.email.toLowerCase().includes(search.toLowerCase()) || p.id.toLowerCase().includes(search.toLowerCase()),
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-dark-text-tertiary mt-1">{filtered.length} total participants</p>
        </div>
            ...
            <tbody className="divide-y divide-dark-border/50">
              {paginated.map((p, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm text-dark-text-secondary font-mono">{p.ticket}</td>
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button
                ...
                className={`w-8 h-8 rounded-lg text-sm transition-colors ${
                  p === page ? 'bg-primary-500/20 text-primary-400' : 'text-dark-text-tertiary hover:text-white hover:bg-dark-surface-tertiary'
                }`}
              >
                {p}
              </button>
