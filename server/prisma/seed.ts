...
async function main() {
  ...
  if (userCount === 0) {
    ...
    await prisma.user.createMany({
      data: [
        { username: 'admin',       email: 'admin@radiantluckydraw.com',      password: adminPassword,          role: 'admin',       isActive: true },
        { username: 'superadmin',  email: 'admin@radiantgroup.com',          password: productionAdminPassword, role: 'super_admin', isActive: true },
        { username: 'operator1',   email: 'operator1@radiantluckydraw.com',  password: operatorPassword,       role: 'operator',    isActive: true },
        { username: 'operator2',   email: 'operator2@radiantluckydraw.com',  password: operatorPassword,       role: 'operator',    isActive: true },
      ],
    });
  ...
  if (announcementCount === 0) {
    ...
    await prisma.announcement.createMany({
      data: [
        { title: 'Welcome!',         message: 'Welcome to the Radiant Lucky Draw event! We hope you have a fantastic experience.', type: 'success', priority: 1, isActive: true, createdBy: admins?.id },
        { title: 'Schedule Update',  message: 'The Grand Prize draw has been moved to 5:00 PM.',                                  type: 'info',    priority: 2, isActive: true, createdBy: admins?.id },
        { title: 'Technical Note',   message: 'Please ensure your QR code is ready when approaching the booth.',                  type: 'info',    priority: 3, isActive: true, createdBy: admins?.id },
      ],
    });
  ...
  console.log(`  Announcements: ${await prisma.announcement.count({ where: { deletedAt: null } })}`);
