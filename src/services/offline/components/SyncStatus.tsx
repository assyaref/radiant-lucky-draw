...
export function SyncStatus({ className = '', compact = false }: SyncStatusProps) {
  ...
  return (
    <div className={`rounded-xl border border-dark-border bg-dark-surface-secondary p-4 ${className}`}>
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between text-xs">
          ...
          <span className={failedCount > 0 ? 'text-red-400 font-medium' : 'text-dark-text-secondary'}>
            {failedCount}
          </span>
        </div>
        {lastSyncAt && (
        ...
        <div className="mt-3 pt-3 border-t border-dark-border">
          ...
          <div className="max-h-20 overflow-y-auto space-y-1">
            {syncErrors.slice(-3).map((err, i) => (
              <p key={i} className="text-xs text-red-400/70 truncate">{err}</p>
            ))}
          </div>
        </div>
      )}
