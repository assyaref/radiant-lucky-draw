...
const mockDraws = Array.from({ length: 15 }, (_, i) => ({
  ...
  status: ['draft', 'active', 'completed', 'cancelled'][Math.floor(Math.random() * 4)] as
...
export default function DrawsPage() {
  ...
  const filtered = mockDraws.filter(
    (d) =>
              ...
              {paginated.map((d, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`px-2 py-0.5 text-xs font-medium rounded-full ${statusColors[d.status]}`}
                    >
                      {d.status}
                    </span>
                  </td>
