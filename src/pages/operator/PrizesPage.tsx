...
const mockPrizes = Array.from({ length: 18 }, (_, i) => ({
  ...
  tier: ['gold', 'silver', 'bronze', 'special'][Math.floor(Math.random() * 4)] as
...
export default function PrizesPage() {
  ...
  const filtered = mockPrizes.filter(
    (p) =>
              ...
              {paginated.map((p, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`px-2 py-0.5 text-xs font-medium rounded-full ${tierColors[p.tier]}`}
                    >
                      {p.tier}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`px-2 py-0.5 text-xs font-medium rounded-full ${p.status === 'active' ? 'bg-success-500/20 text-success-400' : 'bg-danger-500/20 text-danger-400'}`}
                    >
                      {p.status}
                    </span>
                  </td>
